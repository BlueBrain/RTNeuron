                           ...
                           uint32_t seed, void * out )
{
  ...
  switch(len & 15)
  {
  case 15: k4 ^= tail[14] << 16; // falls through
  case 14: k4 ^= tail[13] << 8; // falls through
  case 13: k4 ^= tail[12] << 0;
           k4 *= c4; k4  = ROTL32(k4,18); k4 *= c1; h4 ^= k4;
  // falls through
  case 12: k3 ^= tail[11] << 24; // falls through
  case 11: k3 ^= tail[10] << 16; // falls through
  case 10: k3 ^= tail[ 9] << 8; // falls through
  case  9: k3 ^= tail[ 8] << 0;
           k3 *= c3; k3  = ROTL32(k3,17); k3 *= c4; h3 ^= k3;
  // falls through
  case  8: k2 ^= tail[ 7] << 24; // falls through
  case  7: k2 ^= tail[ 6] << 16; // falls through
  case  6: k2 ^= tail[ 5] << 8; // falls through
  case  5: k2 ^= tail[ 4] << 0; // falls through
           k2 *= c2; k2  = ROTL32(k2,16); k2 *= c3; h2 ^= k2;
 // falls through
  case  4: k1 ^= tail[ 3] << 24; // falls through
  case  3: k1 ^= tail[ 2] << 16; // falls through
  case  2: k1 ^= tail[ 1] << 8; // falls through
  case  1: k1 ^= tail[ 0] << 0;
           k1 *= c1; k1  = ROTL32(k1,15); k1 *= c2; h1 ^= k1;
                           ...
                           const uint32_t seed, void * out )
{
  ...
  case  9: k2 ^= uint64_t(tail[ 8]) << 0;
           k2 *= c2; k2  = ROTL64(k2,33); k2 *= c1; h2 ^= k2; // falls through
