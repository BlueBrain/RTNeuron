...
{
#ifdef USE_CONTINUOUS_LOD
    /** \bug There used to be something wrong with clodThreshold inside depth
        partitioning render passes (related to problems with uniform variables
        in general). Need to check whether it works now or not. */
    ...
    float depth1;
    {
        ...
        cornerRT1 = osg_ProjectionMatrix * vec4(right1, top1, depth1, 1);
        cornerRT1.xyz /= cornerRT1.w;
        cornerLB1 = osg_ProjectionMatrix * vec4(left1, bottom1, depth1, 1);
    ...
    float depth2;
    {
        ...
        cornerRT2 = osg_ProjectionMatrix * vec4(right2, top2, depth2, 1);
        cornerRT2.xyz /= cornerRT2.w;
        cornerLB2 = osg_ProjectionMatrix * vec4(left2, bottom2, depth2, 1);
...
// Compute planes applying model view inverse matrix
vec4 clipVertex;
