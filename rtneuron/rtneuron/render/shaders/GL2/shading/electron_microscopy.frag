...
vec3 noiseDisplacement(vec3 position, vec4 sample, float offset)
{
    vec3 disp;
    vec4 noisevec;
    noisevec = texture3D(noise, position);
    disp.x = dot(noisevec, sample) - offset;
    noisevec = texture3D(noise, position + vec3(noisevec));
    disp.y = dot(noisevec, sample) - offset;
    noisevec = texture3D(noise, position - vec3(noisevec));
    disp.z = dot(noisevec, sample) - offset;
    return disp;
}
...
vec4 shadeFragment()
{
    ...
    color += gl_LightSource[0].diffuse.rgb * baseColor.rgb * lambertTerm;
