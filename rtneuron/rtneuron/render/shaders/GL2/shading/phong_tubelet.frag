...
flat varying vec4 color1;
flat varying vec4 color2;
flat varying vec4 planes[2];
...
#endif
    normalized_eye = e;
    depthCache = 0.5 * (1.0 - gl_ProjectionMatrix[2][2] -
                        gl_ProjectionMatrix[3][2] / (e.z * t));
}
...
vec4 shadeFragment()
{
    ...
    vec3 l = vec3(0, 0, 1);
#endif
    vec4 color = phong(getBaseColor(interpColor), normal, -normalized_eye, l);
