...
{
template <typename T>
void DataIStreamArchive::load_array(array_serializer<T>& a, unsigned int)
{
    _stream >> co::Array<T>(a.address(), a.count());
}
...
template <typename T>
typename boost::enable_if<boost::is_integral<T>>::type DataIStreamArchive::load(
    T& t)
{
