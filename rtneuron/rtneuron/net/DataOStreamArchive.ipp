...
template <typename T>
void DataOStreamArchive::save_array(const array_serializer<T>& a, unsigned int)
{
    _stream << co::Array<const T>(a.address(), a.count());
}
...
template <typename T>
typename boost::enable_if<boost::is_integral<T>>::type DataOStreamArchive::save(
    const T& t)
{
